...
<script type="text/javascript">
				...
				function updatePersonalInfo(){
					$("#ff").form("submit",{
			    		 url:"${pageContext.request.contextPath}/teacher_updatePersonalInfo.action", 
			    		onSubmit:function(){
	    		...
	    		$(function(){
	    			$("input[name='teacher.sex'][value='${currentUser.sex}']").attr("checked",true);
	    		});
		</script>
</head>
<body>
	...
	<div style="width: 400px; padding: 30px 60px; margin-left: 300px">
		<form id="ff" data-options="novalidate:true" method="post">
			...
			<div style="margin-bottom: 20px">
				<div>姓名:</div>
				<input class="easyui-validatebox" name="teacher.realName"
					style="width: 100%; height: 32px" value="${currentUser.realName}"
					required="true">
			</div>
			<div style="margin-bottom: 20px">
				<div>性别:</div>
				   <input type="radio" name="teacher.sex"  class="easyui-validatebox" required="true" value="男" >男
			...
			<div style="margin-bottom: 20px">
				<div>电话号码:</div>
				<input class="easyui-validatebox" name="teacher.phone" id="phone"
					value="${currentUser.phone}" style="width: 100%; height: 32px"
					required="true" >
			</div>
			<div style="margin-bottom: 20px">
				<div>邮箱:</div>
				<input class="easyui-validatebox" name="teacher.email" id="email"

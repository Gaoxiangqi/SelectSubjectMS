...
import com.qgx.selectSubjectMS.utils.MailUtil;
...
public class UserAction extends ActionSupport implements ServletRequestAware {
	...
	public String login() throws Exception {
		...
		UsernamePasswordToken token = new UsernamePasswordToken(user.getUserName(),
				StringUtil.encryptMd5(user.getPassword()));
		try {
			subject.login(token);
		} catch (Exception e) {
			error="用户名或密码错误";
		...
		case "Admin":  {
			Admin currentUser=adminService.findAdminByUserId(user.getId());
			request.getSession().setAttribute("currentUser", currentUser);
			break;
		}
		case "DeptHead":  {
			DeptHead currentUser=deptHeadService.findDeptHeadByUserId(user.getId());
			request.getSession().setAttribute("currentUser", currentUser);
			break;
		}
		case "Teacher":  {
			Teacher currentUser=teacherService.findTeacherByUserId(user.getId());
			request.getSession().setAttribute("currentUser", currentUser);
			break;
		}
		case "Student":  {
			Student currentUser=studentService.findStudentByUserId(user.getId());
			request.getSession().setAttribute("currentUser", currentUser);
			break;
		}
		}
	 ...
	 * 能否进入找回密码操作
	 */
	public String resetPswPage()throws Exception {
		...
		if (curUser == null) {
			...
			case "Admin":  {
				Admin currentUser=adminService.findAdminByUserId(curUser.getId());
				userEmail = currentUser.getEmail();
				break;
			}
			case "DeptHead":  {
				DeptHead currentUser=deptHeadService.findDeptHeadByUserId(curUser.getId());
				userEmail = currentUser.getEmail();
				break;
			}
			case "Teacher":  {
				Teacher currentUser=teacherService.findTeacherByUserId(curUser.getId());
				userEmail = currentUser.getEmail();
				break;
			}
			case "Student":  {
				Student currentUser=studentService.findStudentByUserId(curUser.getId());
				userEmail = currentUser.getEmail();
				break;
			}
			}
			if (email.equals(userEmail)) {
				...
				//输入邮箱和用户邮箱一致，可以进行发送
				String validateCode = StringUtil.getUUID();
				MailUtil.sendMail(email, validateCode);
				request.getSession().setAttribute("validateCode", validateCode);
				request.getSession().setAttribute("unanuthenticUser", curUser);
				return "emailSuccess";
			} else {
				error = "您输入的邮箱有误！";
				return "findPassword";
	 ...
	 * 进入重置密码界面
	 */
	public String toResetPage() throws Exception {
		return "resetPsw";
	}
	/*
	 * 重置密码
	...
	public String resetPsw()throws Exception {
		String session_validateCode = (String)request.getSession().getAttribute("validateCode");
		if (session_validateCode.equals(validateCode)) {
			User curUser = (User)request.getSession().getAttribute("unautheticUser");
	 ...
	 * 题目细节
	 */
	public String titleDetail() throws Exception {
		String subjectId = request.getParameter("id");
		com.qgx.selectSubjectMS.entity.Subject currentSubject = subjectService.getSubjectById(subjectId);
	 ...
	 * 验证码功能
	 */
	public String checkImg() throws Exception {
		...
		// 将生成的字母存入到session中
		ServletActionContext.getRequest().getSession().setAttribute("session_checkCode", sb.toString());
		// 步骤五 绘制干扰线
